...
# Transformer-Only (Supervised Learning) - 5-Fold CV
# - 278 raw events → 5-fold stratified CV
# - Patch-based tokenization (replaces 1D-CNN with linear patch embedding)
# - Train: sliding window augmentation + per-sample normalization
# - Test: center window only + per-sample normalization (Fair Comparison)
# - Per-fold optimal threshold via Youden's J statistic
...
OUTPUT_FOLDER = os.path.join(WAVE_FOLDER, "Results_Transformer_Only_5Fold")
...
BANDPASS_MIN = 2.0
BANDPASS_MAX = 18.0
EPOCHS = 60            # reduced from 80 (with early stopping)
...
PATCH_SIZE = 50                    # 1500 / 50 = 30 tokens
D_MODEL = 128
...
# ============================================================
# Sliding Windows (Train Augmentation)
# ============================================================
...
# ============================================================
# Center Window Only (Test & Unknown - Fair Comparison)
# ============================================================
...
class PositionalEncoding(tf.keras.layers.Layer):
    def __init__(self, max_len, d_model, **kwargs):
        ...
        self.pe = tf.constant(angle_rads[np.newaxis, :, :], dtype=tf.float32)
...
# ============================================================
# Patch Embedding (Replaces 1D-CNN Front-End)
# ============================================================
@tf.keras.utils.register_keras_serializable()
class PatchEmbedding(tf.keras.layers.Layer):
    ...
    def call(self, x):
        ...
        x = tf.reshape(x, [batch, num_patches, self.patch_size])
        return self.proj(x)
...
# ============================================================
def transformer_encoder_block(x, heads, ff_dim, dropout_rate):
    attn_output = tf.keras.layers.MultiHeadAttention(
        num_heads=heads, key_dim=x.shape[-1] // heads
    )(x, x)
    attn_output = tf.keras.layers.Dropout(dropout_rate)(attn_output)
    x1 = tf.keras.layers.LayerNormalization(epsilon=1e-6)(x + attn_output)

    ff_output = tf.keras.layers.Dense(ff_dim, activation='gelu')(x1)
...
def build_transformer_model():
    ...
    # Patch-based tokenization (replaces CNN)
    x = PatchEmbedding(PATCH_SIZE, D_MODEL)(inp)

    seq_len = FIXED_LEN // PATCH_SIZE
    x = PositionalEncoding(max_len=seq_len * 2, d_model=D_MODEL)(x)

    for _ in range(NUM_TRANSFORMER_BLOCKS):
        x = transformer_encoder_block(
            x, TRANSFORMER_HEADS, TRANSFORMER_FF_DIM, TRANSFORMER_DROPOUT
        )
...
X_unknown = (np.array(X_unknown, dtype=np.float32)
             if len(X_unknown) > 0 else np.array([], dtype=np.float32))
y = np.array(y)
...
for train_idx, test_idx in skf.split(X_wave, y):
    ...
    # ---- Train augmentation (same protocol as CNN+Transformer) ----
    ...
    X_test_aug = np.array(
        [center_window(X_test_wave[i]) for i in range(len(X_test_wave))],
        dtype=np.float32
    )
    ...
    best_model_path = os.path.join(OUTPUT_FOLDER, f"best_model_fold{fold_idx}.keras")
    ...
    # Save training history of fold 1 for plot
    if fold_idx == 1:
...
plt.tight_layout()
plt.savefig(os.path.join(OUTPUT_FOLDER, "training_history_fold1.png"), dpi=300)
plt.close()
...
print(classification_report(y, all_fold_pred,
                            target_names=["Earthquake", "Explosion"]))
...
plt.title("Transformer-Only Confusion Matrix (5-Fold Pooled)")
plt.tight_layout()
plt.savefig(os.path.join(OUTPUT_FOLDER, "confusion_matrix_pooled.png"), dpi=300)
plt.close()
...
plt.title("ROC Curve (Transformer-Only, 5-Fold Pooled)")
plt.legend()
plt.tight_layout()
plt.savefig(os.path.join(OUTPUT_FOLDER, "roc_curve_pooled.png"), dpi=300)
plt.close()
...
ax.set_ylabel("Score")
ax.set_title("Per-Fold Metrics (Transformer-Only)")
ax.legend()
ax.set_ylim(0, 1.05)
plt.tight_layout()
plt.savefig(os.path.join(OUTPUT_FOLDER, "per_fold_metrics.png"), dpi=300)
plt.close()
...
if len(X_unknown) > 0:
    ...
    X_unk_cnn = np.array(
        [center_window(w) for w in X_unknown], dtype=np.float32
    )[..., np.newaxis]

    unk_probs = np.zeros(len(X_unknown))
    for fold_i in range(1, N_FOLDS + 1):
        m_path = os.path.join(OUTPUT_FOLDER, f"best_model_fold{fold_i}.keras")
    ...
    pred_df["Classification"] = pred_df["Predicted_Label"].map(
        {0: "Earthquake", 1: "Explosion"}
    )
    pred_df.to_csv(os.path.join(OUTPUT_FOLDER, "predictions.csv"),
...
print(f"\nBest fold (by AUC): Fold {best_fold['fold']} "
      f"(AUC={best_fold['auc']:.4f})")
final_model_path = os.path.join(OUTPUT_FOLDER, "best_transformer_model.keras")
shutil.copy(
    os.path.join(OUTPUT_FOLDER, f"best_model_fold{best_fold['fold']}.keras"),
    final_model_path
)
...
# Training Summary
# ============================================================
with open(os.path.join(OUTPUT_FOLDER, "training_summary.txt"),
          "w", encoding='utf-8') as f:
    f.write("Earthquake / Explosion Classification: "
            "Transformer-Only (5-Fold CV)\n")
    f.write("=" * 60 + "\n")
    f.write("Model: Transformer Encoder with Patch Embedding (NO CNN)\n")
    f.write(f"Patch size: {PATCH_SIZE}, "
            f"Num patches: {FIXED_LEN // PATCH_SIZE}\n")
    f.write(f"Transformer blocks: {NUM_TRANSFORMER_BLOCKS}, "
            f"Heads: {TRANSFORMER_HEADS}, FF dim: {TRANSFORMER_FF_DIM}\n")
    f.write(f"D_MODEL: {D_MODEL}\n")
    f.write(f"Input: Raw 1D waveform ({FIXED_LEN} samples)\n")
