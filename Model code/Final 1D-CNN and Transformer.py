...
# 1D-CNN + Transformer (Supervised Learning) - 5-Fold CV
...
OUTPUT_FOLDER = os.path.join(WAVE_FOLDER, "Results_CNN_Transformer_5Fold")
...
def process_wave(path):
    ...
    tr.detrend("demean")
    tr.detrend("linear")
    tr.filter("bandpass", freqmin=BANDPASS_MIN, freqmax=BANDPASS_MAX, corners=4, zerophase=True)
...
def sliding_windows(wave):
    ...
    for off in WINDOW_OFFSETS:
        ...
        seg = np.interp(np.linspace(0, len(seg) - 1, FIXED_LEN), np.arange(len(seg)), seg)
...
def build_cnn_transformer_model():
    ...
    for _ in range(NUM_TRANSFORMER_BLOCKS):
        x = transformer_encoder_block(x, TRANSFORMER_HEADS, TRANSFORMER_FF_DIM, TRANSFORMER_DROPOUT)

    x = tf.keras.layers.GlobalAveragePooling1D()(x)
    x = tf.keras.layers.Dense(128, activation='gelu', kernel_regularizer=tf.keras.regularizers.l2(1e-4))(x)
...
X_unknown = np.array(X_unknown, dtype=np.float32) if len(X_unknown) > 0 else np.array([], dtype=np.float32)
y = np.array(y)
...
for train_idx, test_idx in skf.split(X_wave, y):
    ...
    early_stop = tf.keras.callbacks.EarlyStopping(
        monitor='val_auc', mode='max', patience=25, restore_best_weights=True
    )
...
plt.title("CNN+Transformer Confusion Matrix (5-Fold Pooled)")
plt.tight_layout()
...
plt.title("ROC Curve (CNN+Transformer, 5-Fold Pooled)")
plt.legend()
plt.tight_layout()
...
ax.set_ylabel("Score")
ax.set_title("Per-Fold Metrics (CNN+Transformer)")
ax.legend()
ax.set_ylim(0, 1.05)
plt.tight_layout()
...
if len(X_unknown) > 0:
    ...
    pred_df.to_csv(os.path.join(OUTPUT_FOLDER, "predictions.csv"), index=False, encoding='utf-8-sig')
...
final_model_path = os.path.join(OUTPUT_FOLDER, "best_cnn_transformer_model.keras")
shutil.copy(
...
# Training Summary
# ============================================================
with open(os.path.join(OUTPUT_FOLDER, "training_summary.txt"), "w", encoding='utf-8') as f:
    f.write("Earthquake / Explosion Classification: 1D-CNN + Transformer (5-Fold CV)\n")
    f.write("=" * 60 + "\n")
    f.write(f"Model: 1D-CNN + Transformer Encoder\n")
