...
# Random Forest with Hand-crafted Features (5-Fold CV)
...
# - Per-fold GridSearchCV + RF → Per-fold optimal threshold → Evaluation
...
OUTPUT_FOLDER = os.path.join(WAVE_FOLDER, "Results_RF_5Fold")
...
plt.title("RF Confusion Matrix (5-Fold Pooled)")
plt.tight_layout()
plt.savefig(os.path.join(OUTPUT_FOLDER, "rf_confusion_matrix_pooled.png"), dpi=300)
plt.close()
...
plt.title("ROC Curve (Random Forest, 5-Fold Pooled)")
plt.legend()
plt.tight_layout()
plt.savefig(os.path.join(OUTPUT_FOLDER, "rf_roc_curve_pooled.png"), dpi=300)
plt.close()
...
ax.set_ylabel("Score")
ax.set_title("Per-Fold Metrics (Random Forest)")
ax.legend()
ax.set_ylim(0, 1.05)
plt.tight_layout()
plt.savefig(os.path.join(OUTPUT_FOLDER, "rf_per_fold_metrics.png"), dpi=300)
plt.close()

# ============================================================
# Feature Importance (Averaged across 5 folds)
# ============================================================
print("\n--- Random Forest Averaged Feature Importance ---")
...
imp_df.to_csv(os.path.join(OUTPUT_FOLDER, "rf_feature_importance.csv"),
              index=False, encoding='utf-8-sig')

top_n = 20
plt.figure(figsize=(10, 8))
top = imp_df.head(top_n)
plt.barh(range(top_n), top['Importance'].values[::-1], color='forestgreen')
plt.yticks(range(top_n), top['Feature'].values[::-1])
plt.xlabel('Averaged Gini Importance (Mean Decrease in Impurity)')
plt.title(f'Top {top_n} Feature Importances (RF 5-Fold Average)')
plt.tight_layout()
...
if len(X_unknown) > 0:
    ...
    pred_df.to_csv(os.path.join(OUTPUT_FOLDER, "rf_predictions.csv"),
                   index=False, encoding='utf-8-sig')
...
# Summary
# ============================================================
with open(os.path.join(OUTPUT_FOLDER, "rf_summary.txt"), "w", encoding='utf-8') as f:
    f.write("Earthquake / Explosion Classification: Random Forest (5-Fold CV)\n")
    f.write("=" * 60 + "\n")
